        ...
        <form action="{{ route('threads.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}"
                       class="text-black w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400 focus:outline-none" required placeholder="Judul Thread">
            </div>

            <div>
                <label class="block text-white font-medium mb-2">Konten</label>
                <textarea name="content" rows="6"
                          class="text-black w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400 focus:outline-none" required>{{ old('content') }}</textarea>
            </div>
